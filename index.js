...
app.post('/webhook', async (req, res) => {
  res.status(200).json({ ok: true }); // Responder inmediatamente
  ...
  try {
    ...
    const existing = await dbGet('pagos', { mp_payment_id: paymentId });
    if (existing) return;
    ...
    // Guardar código
    await dbInsert('codigos', {
      codigo, email, plan,
      vencimiento, estado: 'activo', usos: 0
    });

    // Guardar pago
    await dbInsert('pagos', {
      id: generateId(), codigo, email,
      monto: pago.transaction_amount, plan,
      estado: 'aprobado', mp_payment_id: paymentId.toString()
    });
...
async function handleGetRecipes(body) {
  ...
  if (filtro_categoria) url += `&categoria=eq.${filtro_categoria}`;
  ...
  const paginadas = recetas.slice(inicio, inicio + por_pagina).map(r => ({
    ...r, ...(r.notas || {}), favorita: r.favorita
  }));
...
async function handleLogCalorias(body) {
  ...
  const kcal_total = comidas.reduce((s, c) => s + Number(c.kcal || 0), 0);
  const proteinas = comidas.reduce((s, c) => s + Number(c.proteinas || 0), 0);
  const carbohidratos = comidas.reduce((s, c) => s + Number(c.carbohidratos || 0), 0);
  const grasas = comidas.reduce((s, c) => s + Number(c.grasas || 0), 0);

  const data = { comidas, kcal_total: Math.round(kcal_total), proteinas: Math.round(proteinas), carbohidratos: Math.round(carbohidratos), grasas: Math.round(grasas) };
...
async function handleCreatePreference(body) {
  ...
  const r = await fetch('https://api.mercadopago.com/checkout/preferences', {
    ...
    body: JSON.stringify({
      ...
      back_urls: { success: `${frontendUrl}/pago-exitoso.html`, failure: `${frontendUrl}/pago-fallido.html`, pending: `${frontendUrl}/pago-pendiente.html` },
...
async function handleAdminGetStats(body) {
  ...
  return { ok: true, stats: {
    ...
    ventas_7d: Array.from({ length: 7 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - 6 + i); const ds = d.toISOString().slice(0, 10); return { fecha: ds, cant: pagos.filter(p => p.fecha?.slice(0, 10) === ds).length, monto: pagos.filter(p => p.fecha?.slice(0, 10) === ds).reduce((s, p) => s + Number(p.monto || 0), 0) }; })
  }};
}
